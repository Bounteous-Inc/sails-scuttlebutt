 ...
 * @scottmac/sails-disk
 *
 * @danwilkerson/sails-scuttlebutt
 *
 * Fixtures via adapter for SailsJS
 */
module.exports = (function sailsScuttlebutt () {
  ...
  var adapter = {
    ...
    /* @NOTE
     * This causes the waterline-adapter-tests test "should work when changing
     * a user\'s primary key value". See:
     * @link https://github.com/balderdashy/waterline-adapter-tests/blob/master/interfaces/semantic/update.test.js#L119
     */
    identity: 'sails-scuttlebutt',
    ...
    registerDatastore: function registerDatastore(datastoreConfig, models, cb) {
      ...
      async.each(_.keys(models), function(modelIdentity, next) {
        ...
        var db = new nedb({ inMemoryOnly: true });;
            ...
            if ((val.autoMigrations && val.autoMigrations.unique) || (attributeName === modelDef.primaryKey)) {
              if (val.autoMigrations && val.autoMigrations.unique && (!val.required && (attributeName !== modelDef.primaryKey))) {
                throw new Error('\nIn attribute `' + attributeName + '` of model `' + modelIdentity + '`:\n' +
                                'When using sails-scuttlebutt, any attribute with `unique: true` must also have `required: true`\n');
              }
              db.ensureIndex({
        ...
        db.loadDatabase(function(err) {
          ...
          var configPath = datastore.config.dir;
          var fixturePath = path.resolve(configPath, modelIdentity + '.json');

          Filesystem.readJson({source: fixturePath}, function(err, json) {

            if (err) { return next(err.exit !== 'doesNotExist' ? err : null); }

            var primaryKeyCol = datastore.primaryKeyCols[modelIdentity];

            // Get the possible sequence name for this table.
            var sequenceName = modelIdentity + '_' + primaryKeyCol + '_seq';

            var newRecords = _.map(json, function(newRecord) {

              // If there is a sequence and `null` is being sent in for this
              // record's primary key, set it to the next value of the sequence.
              ...
              // If the primary key col for this table isn't `_id`, set `_id` to
    ...
    create: function create(datastoreName, query, cb) {
      ...
      if (!datastore) { return cb(new Error('Unrecognized datastore: `'+datastoreName+'`,  It doesn\'t seem to have been registered with this adapter (sails-scuttlebutt).')); }
    ...
    update: function update(datastoreName, query, cb) {
      ...
      if (query.valuesToSet[primaryKeyCol]) {
        ...
        if (primaryKeyCol === '_id') { return cb(new Error('Cannot change primary key using sails-scuttlebutt adapter when the primary key column is `_id`.')); }
